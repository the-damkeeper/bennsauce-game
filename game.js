        ...
        for (const m of nearbyMonsters) {
            ...
            if (isColliding(attack, m)) {
                ...
                if (m.hp <= 0 && m.type !== 'testDummy') {
                    ...
                    // Check for job trial boss kill
                    if (m.isTrialBoss) {
                        console.log('Trial boss killed:', m.type, 'isTrialBoss:', m.isTrialBoss);
                        if (typeof checkTrialBossKill === 'function') {
                            checkTrialBossKill(m.type);
                        } else {
                            console.error('checkTrialBossKill function not found!');
                        }
                    }

                    // Handle elite monster death
                    const wasEliteMonster = m.isEliteMonster;
                    const wasMiniBoss = m.isMiniBoss;
                    const wasTrialBoss = m.isTrialBoss;
                    if (wasEliteMonster) {
                        removeEliteMonsterHPBar();
                        currentEliteMonster = null;
                        addChatMessage(`⭐ ELITE ${m.name.toUpperCase()} DEFEATED! ⭐`, 'legendary');
                    } else if (wasMiniBoss || wasTrialBoss) {
                        removeMiniBossHPBar();
                    }
                    ...
                    setTimeout(() => {
                        ...
                        if (index > -1) {
                            m.element.remove();
                            // Remove from spatial grid before removing from array
                    ...
                    // Calculate rewards (elite monsters get huge bonuses)
                    const expMultiplier = wasEliteMonster ? 10 : 1;
                    gainExp(m.exp * expMultiplier);
                    
                    updateQuestProgress(m.type);
                    updateAchievementProgress('kill', m.type);
                    player.stats.totalKills = (player.stats.totalKills || 0) + 1;
                    
                    // Track party kills for achievements
                    const isInParty = typeof getPartyInfo === 'function' && getPartyInfo().partyId;
                    
                    if (isInParty) {
                        player.stats.partyKills = (player.stats.partyKills || 0) + 1;
                        updateAchievementProgress('action_accumulate', 'party_kills');
                        
                        // Track party boss kills
                        if (wasMiniBoss) {
                            player.stats.partyBossKills = (player.stats.partyBossKills || 0) + 1;
                            updateAchievementProgress('action_accumulate', 'party_boss_kills');
                        }
                    }
                    ...
                    if (typeof updateGuildChallengeProgress === 'function') {
                        updateGuildChallengeProgress(wasMiniBoss);
                    }
                    
                    // Check for server-first boss medal when killing a boss
                    if (wasMiniBoss && typeof checkAllBossesKilledMedal === 'function') {
                    ...
                    if (wasEliteMonster) {
                        ...
                        for (let i = 0; i < ticketCount; i++) {
                            setTimeout(() => {
                                createItemDrop('Gachapon Ticket', m.x - 20 + (i * 15), m.y + m.height / 2);
                                updateBestiaryDrop(m.type, 'Gachapon Ticket');
                            }, i * 100);
                        }
                        ...
                        for (let i = 0; i < scrollCount; i++) {
                            setTimeout(() => {
                                createItemDrop('Enhancement Scroll', m.x + 20 + (i * 15), m.y + m.height / 2);
                                updateBestiaryDrop(m.type, 'Enhancement Scroll');
                            }, i * 100);
                        }
                    }
                    ...
                    m.loot.forEach(loot => {
                        ...
                        const dropRateMultiplier = wasEliteMonster ? 3 : 1; // Elite monsters have 3x drop rate
                        if (Math.random() < (loot.rate * DROP_RATE_MODIFIER * dropRateMultiplier)) {
                            if (loot.name === 'Gold') {
                                const baseGoldAmount = Math.floor(Math.random() * (loot.max - loot.min + 1)) + loot.min;
                                const goldAmount = wasEliteMonster ? baseGoldAmount * 20 : baseGoldAmount; // Elite monsters drop 20x gold from regular loot
